...
if __name__ == "__main__":
    ...
    for index, row in enumerate(real_data_here):
        ...
        for item in row:  
            if '.jpg' in item:
                ele = Element()
                ele.addImg(f'./../frame_cache_HD/{item[:-11]}/'+item)
                r.addElement(ele)
            # else:
    ...
    tw = TableWriter(t, '/home/dandans/public_html/PYHTML/frame_cache_HD_html', topName=file_name)
    tw.write()
